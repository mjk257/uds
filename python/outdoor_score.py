...
# Calculate outdoor scores and update DB, use state score if median AQI unavailable
writes = []
...
curves = []
for city in cities.find({"outdoor_score":{"$exists": True}}):
    query = {'name': city['name'], 'state': city['state']}
    normalized_score = int(city['outdoor_score'] + (100 - max["outdoor_score"]))
    values = { "$set": { "outdoor_score": normalized_score}}
    curves.append(UpdateOne(query, values))

cities.bulk_write(curves)